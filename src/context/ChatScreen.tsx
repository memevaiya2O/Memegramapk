...
export default function ChatScreen({ chat, messages, onBack, onSendMessage, onSendMedia, onBlockChat }: ChatScreenProps) {
  ...
  return (
    <div className="flex flex-col h-full bg-[#f0f2f5] dark:bg-[#050505] transition-colors relative overflow-hidden">
      ...
      {/* Header */}
      <header className="p-4 flex items-center justify-between glass sticky top-0 z-30 shadow-sm">
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-4 cursor-pointer group" onClick={() => setShowProfile(true)}>
            <div className="relative">
              {chat.photoUrl ? (
                <img src={chat.photoUrl} alt={chat.first_name} className="w-12 h-12 rounded-2xl object-cover shadow-lg ring-2 ring-white/20" referrerPolicy="no-referrer" />
              ) : (
                <div className="w-12 h-12 bg-primary rounded-2xl flex items-center justify-center text-white font-bold shadow-lg text-xl">
                  {chat.first_name[0]}
                </div>
              )}
            ...
            <div>
              <h3 className="font-bold text-gray-900 dark:text-white leading-tight text-lg group-hover:text-primary transition-colors">
                {chat.first_name}
              </h3>
        ...
        <div className="flex items-center gap-2">
          <button className="p-3 hover:bg-black/5 dark:hover:bg-white/5 rounded-2xl transition-all active:scale-90">
          ...
          <div className="relative">
            <button 
              ...
              className="p-3 hover:bg-black/5 dark:hover:bg-white/5 rounded-2xl transition-all active:scale-90"
            >
      ...
      <div
        ref={scrollRef}
        className="flex-1 overflow-y-auto p-6 space-y-3 z-0 scroll-smooth"
      >
        {messages.map((msg) => (
